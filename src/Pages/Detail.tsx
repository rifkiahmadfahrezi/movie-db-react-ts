...
export default function Detail() {
  ...
  return (
   <> <img 
         ...
         className="fixed inset-0 z-[-1] top-0 w-full object-cover opacity-10" 
         alt="" />
      ...
      <div className="flex flex-col lg:flex-row mt-10 gap-4">
         <figure className="w-full mx-auto lg:w-2/4 md:max-w-fit h-[550px] overflow-hidden shadow-lg">
            <img 
               ...
               className="object-contain w-full h-full lg:bg-stone-500 lg:min-w-[350px]"
         ...
         <figcaption className="w-full lg:w-2/4 mb-10 flex flex-col justify-center">
            <article>
               ...
               <p className="text-md mt-10">{movieData?.overview || <TextLoading num={3}/> }</p>
            </article>

            <div className="flex flex-wrap gap-2 mt-10">
            {movieData?.genres?.map((item : any) => (
               <div className="badge badge-success" key={item.id}>{item.name}</div>
               ))}
            </div>
         </figcaption>
      </div>
